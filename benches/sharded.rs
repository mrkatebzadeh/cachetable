...
impl Workload {
    fn new(name: &'static str, read_frac: f64) -> Self {
        Self { name, read_frac }
    }
}

fn run_workload(c: &mut Criterion, workload: Workload) {
    let mut group = c.benchmark_group(workload.name);

    for threads in [1, 2, 4, 8, 16, 32] {
        group.throughput(Throughput::Elements((NUM_OPS * threads) as u64));

        group.bench_function(format!("Dashmap_{}t", threads), |b| {
            b.iter(|| {
                let table = Arc::new(dashmap::DashMap::<u64, Object>::new());
                {
        ...
        group.bench_function(format!("Cachetable_{}t", threads), |b| {
            b.iter(|| {
...
pub fn workload_a(c: &mut Criterion) {
    let workload_a: Workload = Workload::new("Workload A", 0.5);
    run_workload(c, workload_a)
}
pub fn workload_b(c: &mut Criterion) {
    let workload_b: Workload = Workload::new("Workload B", 0.95);
    run_workload(c, workload_b)
}
pub fn workload_c(c: &mut Criterion) {
    let workload_c: Workload = Workload::new("Workload C", 1.0);
    run_workload(c, workload_c)
}
